...
namespace wenet {
    ...
    void accept_waveform(JNIEnv *env, jobject, jshortArray jWaveform) {
        ...
        LOG(INFO) << "asr accept waveform in ms: " << int(size / 16);
    }

    void set_input_finished() {
        if(!feature_pipeline->input_finished()){
            LOG(INFO) << "asr input finished";
            feature_pipeline->set_input_finished();
        }else{
            LOG(INFO) << "asr input already finished";
        }
    }
    ...
    jboolean decode() {
        ...
        if (state == kEndFeats) {
            LOG(INFO) << "asr endfeats final result: " << result;
            total_result += result;
        } else if (state == kEndpoint) {
            LOG(INFO) << "asr endpoint final result: " << result;
            total_result += result + "，";
            decoder->ResetContinuousDecoding();
        } else {
            if (decoder->DecodedSomething()) {
                LOG(INFO) << "asr partial result: " << result;
            }
        }

        return state == kEndpoint;
    }
    ...
    jstring get_result(JNIEnv *env, jobject) {
        ...
        LOG(INFO) << "asr partial result: " << total_result + result;
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *) {
    ...
    jclass c = env->FindClass("com/intel/automotive/ai/phonon/asr/AsrJniStub");
    if (c == nullptr) {
        return JNI_ERR;
    }
    ...
    static const JNINativeMethod methods[] = {
            ...
            {"acceptWaveform",   "([S)V",                 reinterpret_cast<void *>(wenet::accept_waveform)},
            {"setInputFinished", "()V",                   reinterpret_cast<void *>(wenet::set_input_finished)},
            {"Decode",           "()Z",                   reinterpret_cast<void *>(wenet::decode)},
            {"getResult",        "()Ljava/lang/String;",  reinterpret_cast<void *>(wenet::get_result)},
    };
