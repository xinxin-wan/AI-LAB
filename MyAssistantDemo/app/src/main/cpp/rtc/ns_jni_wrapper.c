 ...
 * nSample:采集数据的频率
 */
JNIEXPORT jint Java_com_example_myassistantdemo_rtc_AudioProcessJni_initiateNSInstance(JNIEnv *env, jclass cls,  jlong nSample, jint nMode){
...
JNIEXPORT jint  Java_com_example_myassistantdemo_rtc_AudioProcessJni_processNS(JNIEnv *env, jclass type, jobject jdirectBuff,jshortArray outframe_,jint sf) {
...
JNIEXPORT jint Java_com_example_myassistantdemo_rtc_AudioProcessJni_destoryNS(JNIEnv *env, jclass cls){

        WebRtcNs_Free(pNS_inst);
}
/*
 * Class:     com_intel_automotive_ai_phonon_rtc_AudioProcessJni
 * Method:    AgcInit
 * Signature: (JJJ)J
 */
JNIEXPORT jlong Java_com_example_myassistantdemo_rtc_AudioProcessJni_AgcInit(JNIEnv *env, jclass cls, jlong minLevel , jlong maxLevel , jlong fs){

        minLevel = 0;
		maxLevel = 255;
		agc_samples  =fs;
		int agcMode  = kAgcModeFixedDigital;
        LOGE("Java_com_intel_automotive_ai_phonon_rtc_AudioProcessJni_AgcInit!  -> %d \n", sizeof(short));
        ...
        LOGE("Java_com_intel_automotive_ai_phonon_rtc_AudioProcessJni_AgcCreate success! \n");
      ...
      LOGE("Java_com_intel_automotive_ai_phonon_rtc_AudioProcessJni_AgcInit success! \n");
...
JNIEXPORT jint  Java_com_example_myassistantdemo_rtc_AudioProcessJni_AgcFun(JNIEnv *env, jclass type, jobject jdirectBuff,jshortArray sArr_, jint frameSize) {
    if(agc_buffer == NULL){
        LOGE("gc_buffer == NULL! \n");
        void* buffer = (*env)->GetDirectBufferAddress(env,jdirectBuff);
        agc_buffer = buffer;
...
JNIEXPORT void JNICALL Java_com_example_myassistantdemo_rtc_AudioProcessJni_AgcFree(JNIEnv *env , jclass  cls){

    WebRtcAgc_Free(agcHandle);
}
JNIEXPORT void JNICALL  Java_com_example_myassistantdemo_rtc_AudioProcessJni_AgcProcess(JNIEnv *env  , jclass type) {
...
/*JNI全处理*/
JNIEXPORT void JNICALL  Java_com_example_myassistantdemo_rtc_AudioProcessJni_NSProcess(JNIEnv *env, jclass type ,jint sf) {
    ...
    if(sf == 32*1000) {
        ...
        if(fpIn == NULL || fpOut == NULL){
            ...
            return;
                ...
                LOGE("NS Java_com_intel_automotive_ai_phonon_rtc_AudioProcessJni_NSProcess  32K   start");
                do {
        ...
        LOGE("NS Java_com_intel_automotive_ai_phonon_rtc_AudioProcessJni_NSProcess  8K   start");
        int len;
        int frameSize =80;
        int i = 0 ;
        const  short *pData    = NULL;
...
JNIEXPORT jint Java_com_example_myassistantdemo_rtc_processNS32K(JNIEnv *env, jclass type, jobject buffer1,
